...
def mnist_generator2(data, batch_size, n_labelled,n_examples, limit=None):
    images, targets = data
    images = images[0:n_examples,:]
    targets = targets[0:n_examples]
    rng_state = numpy.random.get_state()
    numpy.random.shuffle(images)
    numpy.random.set_state(rng_state)
    numpy.random.shuffle(targets)
...
def load(batch_size, test_batch_size, n_examples = 60000, n_labelled=None):
    ...
    with gzip.open('/tmp/mnist.pkl.gz', 'rb') as ff:
        u = pickle._Unpickler(ff)
        u.encoding = 'latin1'
        train_data, dev_data, test_data = u.load()

    return (
        mnist_generator2(train_data, batch_size, n_labelled, n_examples), 

...
def Conv2D(name, input_dim, output_dim, filter_size, inputs, he_init=True, mask_type=None, stride=1, weightnorm=None, biases=True, gain=1.):
    ...
    with tf.name_scope(name):
        ...
        stride_array = [1, stride, stride, 1] # [1, 1, stride, stride]
        inputs = tf.transpose(inputs, [0, 2, 3, 1])  # Switch to NHWC format for CPU compatibitlity
        ...
        if biases:
            ...
            result = tf.nn.bias_add(result, _biases, data_format='NCHW')  # was NCHW, CPU prefers NHWC
